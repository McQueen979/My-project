...
def click_and_hold(duration):
    try:
        ...
        if measurement_mode == 2:
            ...
            start_point = None
            end_point = None
            status_label.config(text="跳跃完成，请按'/'记录下一次起点...")
...
def set_mode(mode):
    ...
    if mode == 1:
        ...
        status_label.config(text="请将鼠标移动到起点位置，按'/'键记录起点...")
...
def handle_key(event):
    ...
    if key == '/':  # 记录起点
        ...
        start_point = (x, y)
        status_label.config(text=f"已记录起点: ({x}, {y}) 像素\n请移动到终点按'*'键")
        ...
        x, y = pyautogui.position()
        end_point = (x, y)
        status_label.config(text=f"已记录终点: ({x}, {y}) 像素\n按'-'键计算距离并执行跳跃")
        ...
        # 计算并显示像素距离
        distance_px = math.sqrt((end_point[0] - start_point[0]) ** 2 + (end_point[1] - start_point[1]) ** 2)
        status_label.config(text=f"像素距离: {distance_px:.2f} 像素")
