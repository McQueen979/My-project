...
# 自动游戏功能
Label(scrollable_frame, text="游戏控制", font=("Arial", 14, "bold")).pack(pady=5)
...
# 调试按钮
debug_btn = tk.Button(scrollable_frame, text="显示调试视图", command=show_debug_view)
debug_btn.pack(pady=10)

# 参数调整面板
Label(scrollable_frame, text="参数调整", font=("Arial", 14, "bold")).pack(pady=10)

# 游戏区域参数
Label(scrollable_frame, text="游戏区域参数", font=("Arial", 10, "bold")).pack(pady=5)
create_param_slider(scrollable_frame, 'GAME_AREA_TOP', '上边界比例', 0.1, 0.4, 0.01, PARAMS['GAME_AREA_TOP'])
create_param_slider(scrollable_frame, 'GAME_AREA_BOTTOM', '下边界比例', 0.6, 0.9, 0.01, PARAMS['GAME_AREA_BOTTOM'])
create_param_slider(scrollable_frame, 'GAME_AREA_LEFT', '左边界比例', 0.05, 0.2, 0.01, PARAMS['GAME_AREA_LEFT'])
create_param_slider(scrollable_frame, 'GAME_AREA_RIGHT', '右边界比例', 0.8, 0.95, 0.01, PARAMS['GAME_AREA_RIGHT'])

# 玩家检测参数
Label(scrollable_frame, text="玩家检测参数", font=("Arial", 10, "bold")).pack(pady=5)
create_param_slider(scrollable_frame, 'PLAYER_MIN_Y', '最小Y比例', 0.2, 0.6, 0.01, PARAMS['PLAYER_MIN_Y'])
create_param_slider(scrollable_frame, 'PLAYER_MIN_AREA', '最小面积', 500, 2000, 100, PARAMS['PLAYER_MIN_AREA'])
create_param_slider(scrollable_frame, 'PLAYER_MAX_AREA', '最大面积', 4000, 10000, 100, PARAMS['PLAYER_MAX_AREA'])

# 目标检测参数
Label(scrollable_frame, text="目标检测参数", font=("Arial", 10, "bold")).pack(pady=5)
create_param_slider(scrollable_frame, 'TARGET_MAX_Y', '最大Y比例', 0.4, 0.8, 0.01, PARAMS['TARGET_MAX_Y'])
create_param_slider(scrollable_frame, 'TARGET_MIN_AREA', '最小面积', 500, 2000, 100, PARAMS['TARGET_MIN_AREA'])
create_param_slider(scrollable_frame, 'TARGET_MAX_AREA', '最大面积', 10000, 30000, 100, PARAMS['TARGET_MAX_AREA'])

# 边缘检测参数
Label(scrollable_frame, text="边缘检测参数", font=("Arial", 10, "bold")).pack(pady=5)
create_param_slider(scrollable_frame, 'CANNY_THRESHOLD1', '阈值1', 10, 100, 5, PARAMS['CANNY_THRESHOLD1'])
create_param_slider(scrollable_frame, 'CANNY_THRESHOLD2', '阈值2', 50, 200, 5, PARAMS['CANNY_THRESHOLD2'])

# 跳跃时间参数
Label(scrollable_frame, text="跳跃时间参数", font=("Arial", 10, "bold")).pack(pady=5)
