...
def detect_two_points():
    """检测屏幕上的两个点（起点和终点）"""
    print("请将鼠标移动到起点位置，1.5秒后开始检测...")
    status_label.config(text="请将鼠标移动到起点位置，1.5秒后开始检测...")
    root.update()
    time.sleep(1.5)  # 缩短为1.5秒
    start_x, start_y = find_pos()
    
    print("请将鼠标移动到终点位置，1.5秒后开始检测...")
    status_label.config(text="请将鼠标移动到终点位置，1.5秒后开始检测...")
    root.update()
    time.sleep(1.5)  # 缩短为1.5秒
    end_x, end_y = find_pos()
    
    # 计算两点之间的像素距离
    distance_px = math.sqrt((end_x - start_x) ** 2 + (end_y - start_y) ** 2)
    # 转换为厘米
    ...
    return start_x, start_y, end_x, end_y, distance_px, distance_cm
...
def auto_play():
    ...
    # 检测两点
    start_x, start_y, end_x, end_y, distance_px, distance_cm = detect_two_points()
